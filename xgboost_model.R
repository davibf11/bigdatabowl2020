...
##Note - only need week 1 for this script, as the only training labels we have are from week 1
...
##Add in coverage
df_xgboost <- df_xgboost %>%
    dplyr::left_join(df_coverages, by = c("game_id" = "game_id",
                                          "play_id" = "play_id"))
...
df_xgboost_use <- df_xgboost_use %>%
    ...
    dplyr::select(coverage,
                  rpca1_safety1, rpca2_bottomcorner, rpca2_topcorner, rpca2_safety2, rpca1_safety2, rpca2_tophook, rpca2_safety1, spca_1_safety1, 
...
#xgb_train <- df_xgboost_use
xgb_train <- df_xgboost_use %>% sample_n(750)
xgb_test <- df_xgboost_use %>% anti_join(xgb_train)
...
xgb_learner <- makeLearner(
  ...
  par.vals = list(
    ...
    lambda = 0.136,
    alpha = 0.457,
    min_child_weight = 2.324,
    colsample_bytree = 0.347
  )
)

xgb_learner_prob <- makeLearner(
  "classif.xgboost",
  predict.type = "prob",
  par.vals = list(
    objective = "multi:softprob",
...
trainTask <- makeClassifTask(data = xgb_train, target = "coverage")
testTask <- makeClassifTask(data = xgb_test, target = "coverage")
#trainTask_tuning <- makeClassifTask(data = df_xgboost_use, target = "coverage")

##Tune parameters code: get everything set up

#xgb_params <- makeParamSet(
#  makeIntegerParam("nrounds", lower = 300, upper = 400),
#  makeNumericParam("eta", lower = .05, upper = .1),
#  makeNumericParam("gamma", lower = .3, upper = .4),
#  makeNumericParam("lambda", lower = 0.1, 0.2),
#  makeNumericParam("alpha", lower = 0.45, upper = 0.5),
#  makeNumericParam("min_child_weight", lower = 2, upper = 4),
#  makeNumericParam("colsample_bytree", lower = 0.3, upper = 0.6)
#)

#control <- makeTuneControlRandom(maxit = 50)
##control <- makeTuneControlGrid(resolution = 1)
...
xgb_model <- train(xgb_learner, trainTask)
xgb_model_prob <- train(xgb_learner_prob, trainTask)

result <- predict(xgb_model, testTask)

##Variable importance
feat_imp <- getFeatureImportance(xgb_model)
feat_imp <- tbl_df(feat_imp$res)

##Plot feature importance

feat_imp_plot <- feat_imp %>%
  ...
  labs(
    title = 'Coverage XGBoost Classifier Feature Importance',
    x = 'Importance', y = NULL
  )

feat_imp_plot
saveRDS(feat_imp_plot, "importance_plot.rds")
...
saveRDS(xgb_model_prob, "xgb_model_prob.rds")
